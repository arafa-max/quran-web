        ...
        {open && (
          <div className="absolute top-full mt-2 w-full bg-[#1c1c1c] border border-[#262626] rounded-xl overflow-hidden shadow-xl z-50">
            {results.suras.length > 0 && (
              <>
                <p className="text-[#F59E0B] text-xs px-3 pt-2 pb-1 uppercase">
                  {t.searchSuras}
                </p>
                {results.suras.map((s) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-[#F59E0B] text-sm font-arabic">
                      {s.name_translate}
                    </span>
                  </button>
                ))}
            ...
            {results.ayahs.length > 0 && (
              <>
                <p className="text-[#F59E0B] text-xs px-3 pt-2 pb-1 uppercase border-t border-[#262626]/50">
                  {t.searchAyahs}
                </p>
                {results.ayahs.map((a) => (
                  <button
                  ...
                  >
                    ...
                    <span className="text-zinc-400 text-xs text-left line-clamp-2">
                      {a.text_ru}
                    </span>
                  </button>
                ))}
            ...
            {results.suras.length === 0 && results.ayahs.length === 0 && (
              <p className="text-zinc-600 text-sm px-3 py-3">
                {t.searchNotFound}
              </p>
            )}
          </div>
